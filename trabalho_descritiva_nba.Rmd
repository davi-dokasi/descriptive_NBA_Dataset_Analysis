...
## BANCO DE DADOS UTILIZADO NO PROEJTO https://www.kaggle.com/datasets/justinas/nba-players-data?select=all_seasons.csv

#getwd() # COMANDO PARA ACHAR O PATH RELACIONADO
setwd("nba_trabalho")

# Pacotes instaladaos para o trabalho
...
# Chamando os pacotes que instalamos
library(readxl)
library(tidyverse)
library(ggplot2)
library(dplyr)
...
knitr::include_graphics("NBA-Transparent-PNG.png")
```
...
## Lendo o dataset 
nba_dataset <- read_csv("all_seasons.csv")
...
knitr::kable(head(top_scorers[order(-top_scorers$total_pts),  
                  c("player_name",
                    "age",
                    "total_gp",
                    "total_pts",
...
# Tabela Mariores Pontuadores por jogo

Tabela dos jogadores com maior razão de pontos por jogo:

```{r, echo = FALSE}

knitr::kable(head(top_scorers[order(-top_scorers$total_pts_gp),  
                  c("player_name",
                    "age",
                    "total_gp",
                    "total_pts",
...
# Médias de pontuações, rebotes e assitências comparadas ao longo do tempo.
...
# Conclusão sobre o King James(LeBron)
...
```{r, echo=FALSE, out.width = "70%",fig.align = "center"}
knitr::include_graphics("lebron-james-lebron.gif")
```
...
A parte mais curiosa dessa pesquisa é o fato de que LeBron James nunca engressou em nenhuma faculdade em sua vida dedicando-se apenas ao basquete, o que nos mostra que não necessario uma faculdade para ter sucesso na carreira do basquete apenas trabalho duro e dedicação.
